...
private[spark] object KubernetesClientUtils extends Logging {
    ...
    if (confDir.isDefined) {
      ...
      val filesSeq: Seq[Option[(String, String)]] = confFiles.map { file =>
        try {
          ...
          case e: MalformedInputException => logDebug(
            s"skipped a non UTF-8 encoded file ${file.getAbsolutePath}.", e)
            None
        }
      }
  ...
  private def listConfFiles(confDir: String, maxSize: Long): Seq[File] = {
    ...
    // and binary files (e.g. jars and zip). As spark properties are resolved in a different step.
