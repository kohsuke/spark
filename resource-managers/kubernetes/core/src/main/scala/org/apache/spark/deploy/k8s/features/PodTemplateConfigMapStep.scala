...
private[spark] class PodTemplateConfigMapStep(
  ...
  if (preferredConfigmapName.length <= KUBERNETES_MAX_NAME_LENGTH) {
    ...
    val randomId = KubernetesUtils.uniqueID(clock = clock)
    val shorterTemplateConfigmapName = s"spark-$randomId-$POD_TEMPLATE_CONFIGMAP"
    logWarning(s"The pod template configmap name preferably be $preferredConfigmapName," +
      s"but this is too long (must be <= $KUBERNETES_MAX_NAME_LENGTH characters). Falling back " +
      s"to use $shorterTemplateConfigmapName as the pod template configmap name.")
