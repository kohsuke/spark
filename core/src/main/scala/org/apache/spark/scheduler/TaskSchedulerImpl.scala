...
private[spark] class TaskSchedulerImpl(
  ...
  override def executorDecommission(
    ...
    synchronized {
      ...
      // Don't bother noting decommissioning for executors that don't have running tasks
      // to prevent a race where the `executorDecommission` can be called after `removeExecutor`
      if (executorIdToRunningTaskIds.contains(executorId) &&
          (oldDecomInfo.isEmpty || !oldDecomInfo.get.isHostDecommissioned)) {
