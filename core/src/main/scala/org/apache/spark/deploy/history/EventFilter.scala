 ...
 *
 * The implementation of EventFilterBuilder should also provide some statistic (approximate is OK)
 * to determine the rate of events being filtered in, so that caller can decide it's good to
 * continue and filter the events using the [[EventFilter]] instance or give up filtering the
 * events since there's no value filtering events if the rate is such so.
 */
