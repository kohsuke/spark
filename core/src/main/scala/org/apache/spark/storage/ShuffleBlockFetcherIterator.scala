...
final class ShuffleBlockFetcherIterator(
  ...
  private[this] val hostLocalBlocksByExecutor =
    scala.collection.mutable.LinkedHashMap[BlockManagerId, Seq[(BlockId, Long)]]()
  ...
  private[this] def partitionBlocksByFetchMode(): ArrayBuffer[FetchRequest] = {
    ...
    for ((address, blockInfos) <- blocksByAddress) {
      ...
      if (address.executorId == blockManager.blockManagerId.executorId) {
        ...
        hostLocalBlockBytes += blockInfos.map(_._2).sum
      } else {
  ...
  private def checkBlockSizes(blockInfos: Seq[(BlockId, Long)]): Unit = {
    blockInfos.foreach { case (blockId, size) => assertPositiveBlockSize(blockId, size) }
  }
    ...
    while (result == null) {
      ...
      result match {
        case r @ SuccessFetchResult(blockId, address, size, buf, isNetworkReqDone) =>
          ...
          if (!localBlocks.contains(blockId) && !hostLocalBlocks.contains(blockId)) {
            bytesInFlight -= size
          }
          if (isNetworkReqDone) {
            reqsInFlight -= 1
