...
object FileCommitProtocol extends Logging {
  ...
  def instantiate(
      ...
      customizeOutputPath: Boolean = false): FileCommitProtocol = {
    ...
    // dynamicPartitionOverwrite: Boolean, customOutputPath: Boolean).
    // If that doesn't exist, try the one with (jobId: string, outputPath: String,
    // dynamicPartitionOverwrite: Boolean).
    // If that still does't exist, try the one with (jobId: string, outputPath: String)
    try {
      ...
      ctor.newInstance(jobId, outputPath, dynamicPartitionOverwrite.asInstanceOf[java.lang.Boolean],
        customizeOutputPath.asInstanceOf[java.lang.Boolean])
    } catch {
