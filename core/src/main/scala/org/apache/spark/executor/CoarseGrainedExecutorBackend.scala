...
import org.apache.spark.scheduler.{DecommissionInfo, ExecutorLossReason, TaskDescription}
...
private[spark] class CoarseGrainedExecutorBackend(
        ...
        if (decommissioned) {
          ...
          driver match {
            case Some(endpoint) =>
              ...
              endpoint.send(DecommissionExecutor(executorId, DecommissionInfo(msg, false)))
            case _ =>
  ...
  private def decommissionSelf(): Boolean = {
    ...
    try {
      ...
      if (driver.nonEmpty) {
        driver.get.askSync[Boolean](DecommissionExecutor(executorId, DecommissionInfo(msg, false)))
      } else {
