...
private[spark] class AppStatusStore(
  ...
  def parseErrorMessage(errorMessage: String): Option[v1.ExceptionFailure] = {
    errorMessage.split("\\r?\\n")
      ...
      .map(e => {
        // HighLevelException: MidLevelException: LowLevelException: Message
        // Use LowLevelException as exception type
        val s = e.split("(?<=Exception|Error): ")
        val (exceptionType, message) =
        ...
        new v1.ExceptionFailure(exceptionType, message, errorMessage)
      })
  }
