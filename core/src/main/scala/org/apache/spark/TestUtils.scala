...
private[spark] object TestUtils {
   ...
   */
  def getAbsolutePathFromCommand(program: String): Option[String] = {
    if (program.split(File.separator, 2).length == 1 &&
        JavaFiles.isRegularFile(Paths.get(program)) &&
        JavaFiles.isExecutable(Paths.get(program))) {
      Some(program)
    } else {
      sys.env("PATH").split(Pattern.quote(File.pathSeparator))
        .map(path => Paths.get(s"${StringUtils.strip(path, "\"")}${File.separator}$program"))
