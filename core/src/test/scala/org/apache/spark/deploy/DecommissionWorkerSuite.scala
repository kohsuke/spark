...
import org.apache.spark.scheduler.{SparkListener, SparkListenerStageSubmitted, SparkListenerTaskEnd, SparkListenerTaskStart, TaskInfo}
...
class DecommissionWorkerSuite
  ...
  test("decommission workers should not result in job failure") {
    ...
    sc.addSparkListener(new RootStageAwareListener {
      override def onTaskStart(taskStart: SparkListenerTaskStart): Unit = {
        if (!isInterestingTask(taskStart.taskInfo, taskStart.stageId, taskStart.stageAttemptId)) {
          return
        }
      ...
      override def onTaskEnd(taskEnd: SparkListenerTaskEnd): Unit = {
        if (!isInterestingTask(taskEnd.taskInfo, taskEnd.stageId, taskEnd.stageAttemptId)) {
          return
        }
  ...
  test("decommission workers ensure that shuffle output is regenerated even with shuffle service") {
    ...
    try {
      val executorIdToWorkerInfo = getExecutorToWorkerAssignments
      val tasksSeen = new ConcurrentLinkedQueue[String]()
  ...
  test("decommission workers ensure that fetch failures lead to rerun") {
    ...
    val jobResult = sc.parallelize(1 to 2, 2)
    ...
    assert(tasksSeen.size() === 6, s"Expected 5 tasks but got $tasksSeen")
  }

  private class RootStageAwareListener extends SparkListener {
    protected var rootStageId: Option[Int] = None

    protected def isInterestingTask(taskInfo: TaskInfo, stageId: Int,
      stageAttemptId: Int): Boolean = {
      val message = s"task ${taskInfo.id} on executor ${taskInfo.executorId} " +
        s"with status ${taskInfo.status}'s stage id $stageId " +
        s"(attempt $stageAttemptId)"
      val isInteresting = (rootStageId.isDefined && rootStageId.get == stageId)
      logInfo(s"$message is ${isInteresting} interesting: $rootStageId'")
      isInteresting
    }
  ...
  private def getExecutorToWorkerAssignments = {
