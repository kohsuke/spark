...
class ShuffleBlockFetcherIteratorSuite extends SparkFunSuite with PrivateMethodTester {
  ...
  test("successful 3 local + 4 host local + 2 remote reads + 1 host local without local dir") {
    ...
    val blocksByAddress = Seq[(BlockManagerId, Seq[(BlockId, Long)])](
      (localBmId, localBlocks.keys.map(blockId => (blockId, 1L)).toSeq),
      (remoteBmId, remoteBlocks.keys.map(blockId => (blockId, 1L)).toSeq),
      (hostLocalBmIdWithLocDir, hostLocalBlocks.keys.map(blockId => (blockId, 1L)).toSeq),
