...
import org.apache.spark.scheduler.{DecommissionInfo, SparkListener, SparkListenerTaskEnd, SparkListenerTaskStart}
...
class BlockManagerDecommissionSuite extends SparkFunSuite with LocalSparkContext
  ...
  test(s"verify that an already running task which is going to cache data succeeds " +
    ...
    sched.decommissionExecutor(execToDecommission, DecommissionInfo("", true))
