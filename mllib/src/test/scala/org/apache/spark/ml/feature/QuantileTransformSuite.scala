...
class QuantileTransformSuite extends MLTest with DefaultReadWriteTest {
  ...
  @transient var sparseData: Array[Vector] = _
   ...
   */
    ...
    /*
      ...
      zeros = np.zeros([10, 2], dtype=X.dtype)
      X2 = np.concatenate((X, zeros))
      X2 = sparse.csc_matrix(X2)

      qt2 = QuantileTransformer(n_quantiles=3, ignore_implicit_zeros=True)
      qtm2 = qt2.fit(X2)

      >>> np.array_equal(qtm.references_, qtm2.references_)
      True
      >>> np.array_equal(qtm.quantiles_, qtm2.quantiles_)
      True
     */
    ...
    assert(qtm2.references ~== qtm.references absTol 1E-5)
    assert(qtm2.quantiles(0) ~== qtm.quantiles(0) absTol 1E-5)
    assert(qtm2.quantiles(1) ~== qtm.quantiles(1) absTol 1E-5)
    testTransformer[(Vector, Vector)](df2, qtm2, "transformed", "expected")(
      assertResult)
    ...
    /*
      ...
      array([[ 0. , -4. ],
             ...
             [ 4. ,  0.5]])
     */
    val qt3 = new QuantileTransform()
      .setInputCol("features")
      .setOutputCol("transformed")
      .setNumQuantiles(3)
      .setSkipZero(false)
    val qtm3 = qt3.fit(df2)
    assert(qtm3.references ~== Vectors.dense(0.0, 0.5, 1.0) absTol 1E-5)
    assert(qtm3.quantiles(0) ~== Vectors.dense(0.0, 0.0, 4.0) absTol 1E-5)
    assert(qtm3.quantiles(1) ~== Vectors.dense(-4.0, 0.0, 0.5) absTol 1E-5)
  }
  ...
  test("gaussian: fit & transform") {
    ...
    /*
      ...
      array([[-5.19933758,  5.19933758],
             ...
             [ 0.        ,  0.        ]])
     */
    val df2 = (data ++ zeros).zip(gaussianRes ++ zeros)
      .toSeq.toDF("features", "expected")
    val qt2 = new QuantileTransform()
      .setDistribution("gaussian")
      .setInputCol("features")
      .setOutputCol("transformed")
