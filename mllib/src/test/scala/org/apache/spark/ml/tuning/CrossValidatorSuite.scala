...
class CrossValidatorSuite
  ...
  override def beforeAll(): Unit = {
    super.beforeAll()
    dataset = sc.parallelize(generateLogisticInput(1.0, 1.0, 100, 42), 1).toDF()
    val dfWithRandom = dataset.withColumn("random", rand(100L))
    val foldCol = when(col("random") < 0.33, 0).when(col("random") < 0.66, 1).otherwise(2)
    datasetWithFold = dfWithRandom.withColumn("fold", foldCol).drop("random")
  }
