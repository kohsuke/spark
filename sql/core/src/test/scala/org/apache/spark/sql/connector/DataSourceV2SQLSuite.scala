...
class DataSourceV2SQLSuite
  ...
  test("ShowNamespaces: default v2 catalog is not set") {
    ...
    assert(exception.getMessage.contains("SHOW NAMESPACES is not supported with v1 table"))
  }
  ...
  test("REPLACE TABLE: session catalog") {
    ...
    assert(e.message.contains("REPLACE TABLE is not supported with v1 table"))
  }
