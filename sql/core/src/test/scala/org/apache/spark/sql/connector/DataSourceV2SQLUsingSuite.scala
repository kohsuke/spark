...
class DataSourceV2SQLUsingSuite extends QueryTest with SharedSparkSession {
  ...
  test("CREATE TABLE with a mismatched partitioning") {
    withTable("t") {
      ...
      assert(e.getMessage.contains("partitioning mismatch"))
  ...
  test("ALTER TABLE") {
    withTable("t") {
      ...
      intercept[IllegalArgumentException](sql("ALTER TABLE t DROP COLUMN i"))
    }
  }

  test("ALTER TABLE with table supporting schema changing") {
    withTable("t") {
      sql(s"CREATE TABLE t(i INT) USING ${classOf[RequireSchemaV2Source].getName}")
...
class ReadWriteV2Source extends TableProvider {
  ...
  override def getTable(
    ...
    if (schema.isPresent && schema.get != ReadWriteV2Source.table.schema) {
      throw new IllegalArgumentException("schema mismatch")
    }
    if (partitions.isPresent && partitions.get.nonEmpty) {
      throw new IllegalArgumentException("partitioning mismatch")
    }
...
class PartitionedV2Source extends TableProvider {
  ...
  override def getTable(
      ...
      partitions: Optional[Array[Transform]],
      properties: util.Map[String, String]): Table = {
    if (schema.isPresent && schema.get != PartitionedV2Source.table.schema) {
      throw new IllegalArgumentException("schema mismatch")
    }
    val actualPartitioning = PartitionedV2Source.table.partitioning
    if (partitions.isPresent && !partitions.get.sameElements(actualPartitioning)) {
      throw new IllegalArgumentException("partitioning mismatch")
    }
...
class ReadOnlyV2Source extends TableProvider {
  override def getTable(
    ...
    assert(!partitions.isPresent)
    val actualSchema = new StructType().add("i", "int")
    val actualPartitions = Array.empty[Transform]
    val properties = util.Collections.emptyMap[String, String]
    val table = new InMemoryTable("read-only", actualSchema, actualPartitions, properties) {
...
class WriteOnlyV2Source extends TableProvider {
  override def getTable(
    ...
    val actualPartitions = Array.empty[Transform]
    val properties = util.Collections.emptyMap[String, String]
    new InMemoryTable("write-only", actualSchema, actualPartitions, properties) {
      override def capabilities: util.Set[TableCapability] = {
...
class RequireSchemaV2Source extends TableProvider {
  override def getTable(
      schema: Optional[StructType],
      partitions: Optional[Array[Transform]],
      properties: util.Map[String, String]): Table = {
    if (!schema.isPresent) throw new UnsupportedOperationException
    new InMemoryTable("require-schema", schema.get(), partitions.orElse(Array.empty), properties) {}
  }
}
