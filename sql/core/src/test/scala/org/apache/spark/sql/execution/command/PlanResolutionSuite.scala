...
import org.apache.spark.sql.catalyst.analysis.{AnalysisTest, NoSuchTableException, ResolveCatalogs, ResolveTables}
import org.apache.spark.sql.catalyst.catalog.{BucketSpec, CatalogStorageFormat, CatalogTable, CatalogTableType}
import org.apache.spark.sql.catalyst.parser.CatalystSqlParser
import org.apache.spark.sql.catalyst.plans.logical.{AlterTable, CreateTableAsSelect, CreateV2Table, DropTable, LogicalPlan}
...
class PlanResolutionSuite extends AnalysisTest {
  ...
  test("alter table: alter table properties") {
    ...
    val sql1 = "ALTER TABLE non_exist SET TBLPROPERTIES ('test' = 'test')"
    val sql2 = "ALTER TABLE non_exist UNSET TBLPROPERTIES ('test')"

    val parsed1 = parseAndResolve(sql1)
    val parsed2 = parseAndResolve(sql2)

    // For non-existing tables, we leave the statement unchanged, and fail with table not found
    // at the end of analysis.
    assert(parsed1.isInstanceOf[AlterTableSetPropertiesStatement])
    assert(parsed2.isInstanceOf[AlterTableUnsetPropertiesStatement])
  }
