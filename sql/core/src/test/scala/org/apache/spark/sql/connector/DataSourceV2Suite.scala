...
import org.apache.spark.sql.connector.catalog.{SupportsDirectWrite, SupportsRead, Table, TableCapability, TableProvider}
...
class DirectWriteV2Source extends SimpleWritableDataSource with SupportsDirectWrite {
  override def inferSchema(options: CaseInsensitiveStringMap): StructType = {
