...
class FileDataSourceV2Suite extends QueryTest with SharedSparkSession {
  ...
  test("Data type validation should check data schema only") {
    withTempPath { dir =>
      ...
      val options = Map("path" -> pathName).asJava
      ...
      val partitioning = provider.inferPartitioning(
        expectedDataSchema, new CaseInsensitiveStringMap(options))
      val table = provider.getTable(expectedDataSchema, partitioning, options)
  ...
  test("Returns correct data schema when user specified schema contains partition schema") {
    withTempPath { dir =>
      ...
      val provider = new DummyFileSourceV2()
      val partitioning = provider.inferPartitioning(
        userSpecifiedSchema, new CaseInsensitiveStringMap(options))
      val table = provider.getTable(userSpecifiedSchema, partitioning, options)
        .asInstanceOf[DummyFileTable]
      assert(table.dataSchema == expectedDataSchema)
    }
  }
