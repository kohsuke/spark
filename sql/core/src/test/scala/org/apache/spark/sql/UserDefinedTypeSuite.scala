...
class UserDefinedTypeSuite extends QueryTest with SharedSparkSession with ParquetTest
  ...
  test("Allow merge UserDefinedType into a native DataType") {
    ...
    def write(dataset: Dataset[XMLGregorianCalendarRecord],
    ...
    val data = Seq(
      XMLGregorianCalendarRecord(new XMLGregorianCalendarImpl(gregorianCalendar))
    )
    ...
    val records = spark.read.load(tempDir).as[TimestampRecord].collect()

    assert(records.length === 2)
    assert(records(0).dt === timestamp)
    assert(records(1).dt === timestamp)
  }
}
