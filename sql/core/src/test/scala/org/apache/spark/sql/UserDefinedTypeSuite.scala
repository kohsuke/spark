...
class UserDefinedTypeSuite extends QueryTest with SharedSparkSession with ParquetTest
  ...
  test("Allow merge UserDefinedType into a native DataType") {
    ...
    case class Record(dt: XMLGregorianCalendar)
    ...
    def write(dataset: DataFrame, directory: String): Unit = dataset
      .write
    ...
    val data: Seq[Record] = Seq(
      Record(new XMLGregorianCalendarImpl(gregorianCalendar))
    )
    ...
    assert(records(0).getAs[Timestamp]("dt") === timestamp)
    assert(records(1).getAs[Timestamp]("dt") === timestamp)
  }
}
