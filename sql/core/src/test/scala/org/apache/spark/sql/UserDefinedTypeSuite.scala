...
class UserDefinedTypeSuite extends QueryTest with SharedSparkSession with ParquetTest
  ...
  test("Allow merge UserDefinedType into a native DataType") {
    ...
    withTempDir { dir =>
      ...
      // Write it to Parquet
      calendarDf.write.parquet(dir.getCanonicalPath)
      ...
      assert(timestampDf.schema.merge(calendarDf.schema) === timestampDf)
      assert(calendarDf.schema.merge(timestampDf.schema) === timestampDf)
