...
class ResolveCatalogsForV1Commands(val catalogManager: CatalogManager, conf: SQLConf)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
    ...
    //       table provider, which is implemented in sql/core and relies on a v1 interface
    //       `DataSourceRegister`.
  ...
  private def getCatalogAndIdent(tblName: Seq[String]): (CatalogPlugin, Identifier) = {
    tblName match {
      case CatalogAndRestNameParts(Left(catalog), restNameParts) =>
        catalog -> restNameParts.asIdentifier
      case CatalogAndRestNameParts(Right(catalog), restNameParts) =>
        catalog -> restNameParts.asIdentifier
    }
  }
}
