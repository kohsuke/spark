...
class ResolveSessionCatalog(
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
    ...
    case ShowColumnsStatement(tableName, namespace) =>
      val v1TableName = namespace match {
        case None => parseV1Table(tableName, "SHOW COLUMNS")
        case Some(namespace1) => parseV1Table(namespace1 ++: tableName, "SHOW COLUMNS")
      }
      ShowColumnsCommand(None, v1TableName.asTableIdentifier)
