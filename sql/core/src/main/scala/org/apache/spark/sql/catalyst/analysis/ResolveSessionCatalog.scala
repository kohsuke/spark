...
class ResolveSessionCatalog(
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
    ...
    case ShowTablePropertiesStatement(tableName, propertyKey) =>
      val v1TableName = parseV1Table(tableName, "SHOW TBLPROPERTIES")
