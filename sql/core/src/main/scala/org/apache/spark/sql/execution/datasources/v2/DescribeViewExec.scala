...
import org.apache.spark.sql.connector.catalog.ViewReservedProperties
...
case class DescribeViewExec(
  ...
  private lazy val tableProperties =
    view.properties.asScala.toMap
        .filter(ViewReservedProperties.RESERVED_PROPERTIES.contains)
        .map(p => p._1 + "=" + p._2)
        .mkString("[", ", ", "]")
