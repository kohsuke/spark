...
class ResolveTables(val catalogManager: CatalogManager)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
    ...
    case DescribeColumnStatement(
         CatalogAndTable(catalog, tblName, Right(table)), colNameParts, isExtended) =>
      throw new AnalysisException("Describing columns is not supported for v2 tables.")

    // blindly convert `DescribeColumnStatement` to v1 `DescribeColumnCommand` because it can
    // describe view/temp view as well.
    case DescribeColumnStatement(tblName, colNameParts, isExtended) =>
      DescribeColumnCommand(tblName.toV1Identifier, colNameParts, isExtended)
  }
