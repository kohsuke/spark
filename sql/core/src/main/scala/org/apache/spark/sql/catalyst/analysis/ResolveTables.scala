...
class ResolveTables(val catalogManager: CatalogManager)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
    ...
    case AlterTableAlterColumnStatement(
      ...
      throw new AnalysisException("ALTER COLUMN is not supported with the session catalog.")
    ...
    case AlterTableRenameColumnStatement(
      ...
      throw new AnalysisException("RENAME COLUMN is not supported with the session catalog.")
    ...
    case AlterTableDropColumnsStatement(
         CatalogAndTable(catalog, tblName, Left(v1Table)), cols) =>
      throw new AnalysisException("DROP COLUMN is not supported with the session catalog.")
    ...
    case DeleteFromStatement(
      ...
      throw new AnalysisException("DELETE FROM is not supported with the session catalog.")
    ...
    case DescribeTableStatement(
         CatalogAndTable(catalog, tblName, Left(v1Table)), partitionSpec, isExtended) =>
