  ...
  private def createLocalReader(plan: SparkPlan): LocalShuffleReaderExec = {
    plan match {
      case c @ CoalescedShuffleReaderExec(q: QueryStageExec, _) =>
        val partitionIndices = getPartitionStartIndices(q, Some(c.partitionStartIndices.length))
        LocalShuffleReaderExec(c.child, partitionIndices)
      case q: QueryStageExec =>
        val partitionIndices = getPartitionStartIndices(q, None)
        LocalShuffleReaderExec(q, partitionIndices)
    }
  }
