...
class CacheManager extends Logging with AdaptiveSparkPlanHelper {
  ...
  def uncacheByCondition(
    ...
    val planToUncache = cachedData.filter(condition)
    this.synchronized {
      cachedData = cachedData.filterNot(cd => planToUncache.exists(_.eq(cd)))
    }
    planToUncache.foreach { _.cachedRepresentation.cacheBuilder.clearCache(blocking)}
  }
