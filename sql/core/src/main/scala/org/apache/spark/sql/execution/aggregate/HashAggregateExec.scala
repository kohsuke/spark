...
case class HashAggregateExec(
  ...
  //
  // Note: The difference from `CodeGenerator.splitExpressions` is that we define an independent
  // function for each aggregation expression. For example, in a query
  // `SELECT SUM(a), AVG(a) FROM VALUES(1) t(a)`, we add two split functions
  // for `SUM(a)` and `AVG(a)`.
  private def splitAggregateExpressions(
    ...
    val inputVars = aggExprs.map { aggExprsInAgg =>
      ...
      val paramLength = CodeGenerator.calculateParamLength(inputVarsInAgg.map(_._2))
          ...
          makeSplitAggFunctions = {
            unsafeRowBufferEvals.zipWithIndex.map { case (rowBufferEvalsInAgg, i) =>
              ...
              val updateRowBuffers = rowBufferEvalsInAgg.zipWithIndex.map { case (ev, j) =>
                val updateExpr = boundUpdateExprsInAgg(bufferOffset + j)
                val dt = updateExpr.dataType
                val nullable = updateExpr.nullable
                CodeGenerator.updateColumn(unsafeRowBuffer, dt, bufferOffset + i, ev, nullable)
              }
              s"""
                 |// evaluate aggregate function
          ...
          val splitAggCode = splitAggregateExpressions(
            ...
            makeSplitAggFunctions = {
             fastRowEvals.zipWithIndex.map { case (fastRowEvalsInAgg, i) =>
               ...
               // All the update code for aggregation buffers should be placed in the end
               // of each aggregation function code.
                val updateRowBuffer = fastRowEvalsInAgg.zipWithIndex.map { case (ev, i) =>
                  val updateExpr = boundUpdateExprsInAgg(bufferOffset + i)
                  val dt = updateExpr.dataType
                  val nullable = updateExpr.nullable
                  CodeGenerator.updateColumn(fastRowBuffer, dt, bufferOffset + i, ev, nullable,
