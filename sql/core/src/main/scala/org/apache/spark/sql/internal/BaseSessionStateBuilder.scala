...
import org.apache.spark.sql.catalyst.analysis.{Analyzer, FunctionRegistry, ResolveCatalogs, ResolveTables}
  ...
  protected def analyzer: Analyzer = new Analyzer(catalog, v2SessionCatalog, conf) {
    override val extendedResolutionRules: Seq[Rule[LogicalPlan]] =
      new FindDataSourceTable(session) +:
        ...
        new ResolveCatalogs(catalogManager, conf) +:
        new ResolveTables(catalogManager) +:
