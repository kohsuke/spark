...
import org.apache.spark.sql.connector.catalog.{SessionConfigSupport, SupportsExternalMetadata, Table, TableProvider}
...
private[sql] object DataSourceV2Utils extends Logging {
  ...
  def getTableFromProvider(
    ...
    userSpecifiedSchema match {
      case Some(schema) =>
        if (!provider.isInstanceOf[SupportsExternalMetadata]) {
          throw new UnsupportedOperationException(
            s"${provider.getClass.getSimpleName} source does not support user-specified schema.")
        }
        val p = provider.asInstanceOf[SupportsExternalMetadata]
        p.getTable(schema, p.inferPartitioning(options), options.asCaseSensitiveMap())

      case None => provider.getTable(options.asCaseSensitiveMap())
    }
  }
}
