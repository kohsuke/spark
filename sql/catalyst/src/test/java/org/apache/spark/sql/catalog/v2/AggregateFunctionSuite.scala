...
class AggregateFunctionSuite extends SparkFunSuite {
  ...
  test("Test associative iavg(long)") {
    ...
    val bound = IntegralAverage.bind(new StructType().add("foo", LongType, nullable = false))
    assert(bound.isInstanceOf[AssociativeAggregateFunction[_, _]])
    val udaf = bound.asInstanceOf[AssociativeAggregateFunction[Serializable, _]]
...
object IntAverage extends AssociativeAggregateFunction[(Int, Int), Int] {
  override def name(): String = "iavg"
...
object LongAverage extends AssociativeAggregateFunction[(Long, Long), Long] {
  override def name(): String = "iavg"
