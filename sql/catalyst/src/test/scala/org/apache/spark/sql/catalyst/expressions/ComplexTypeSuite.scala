...
class ComplexTypeSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("SPARK-22693: CreateNamedStruct should not use global variables") {
    ...
    val nonNullInputs = differentDataTypes.map(Literal.default)
    val nullInputs = differentDataTypes.map(Literal.create(null, _))
    ...
    // should throw an exception if fieldName = null
    assert(intercept[RuntimeException] {
      AddField(nonNullStruct, null, Literal.create(2, IntegerType))
    }.getMessage == "requirement failed: fieldName cannot be null")
