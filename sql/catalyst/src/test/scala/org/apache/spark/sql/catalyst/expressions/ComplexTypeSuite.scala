...
class ComplexTypeSuite extends SparkFunSuite with ExpressionEvalHelper {
...

  {
    ...
    test(testNamePrefix +
      "checkInputDataTypes should fail if first argument is not a struct dataType") {
      nonNullInputs
        .foreach {
          case inputStruct if inputStruct.dataType.typeName == "struct" =>
            val result = AddFields(inputStruct :: Literal("b") :: Literal(2) :: Nil)
              .checkInputDataTypes()
    ...
    test(testNamePrefix + "checkInputDataTypes should fail if fieldName is not a string") {
      nonNullInputs.foreach {
        case inputField if inputField.dataType.typeName != StringType.typeName =>
          assert({
            val result = AddFields(nonNullStruct :: inputField :: Literal.create(2, IntegerType) ::
              Nil).checkInputDataTypes()
            val expected = TypeCheckResult.TypeCheckFailure(
              s"Only foldable ${StringType.catalogString} expressions are allowed to appear at " +
                "even position.")
            result == expected
          })
        case inputField =>
    ...
    test(testNamePrefix + "checkInputDataTypes should fail if fieldName is null string") {
      assert({
        val result = AddFields(nonNullStruct :: Literal.create(null, StringType) ::
          Literal.create(2, IntegerType) :: Nil).checkInputDataTypes()
        val expected = TypeCheckResult.TypeCheckFailure("Field name should not be null.")
        result == expected
      })
    }
    ...
    test(testNamePrefix + "should replace field with non-null value in struct") {
      nonNullInputs.foreach { inputField =>
        val expectedValue = create_row(1, inputField.value, true)
        val expectedSchema = StructType(schema.updated(1, StructField("b", inputField.dataType,
          inputField.nullable)))
    ...
    test(testNamePrefix + "should add and replace multiple fields") {
      ...
      val expectedSchema = StructType(schema.updated(2, StructField("c", IntegerType,
        nullable = false))).add(StructField("d", IntegerType, nullable = false))
    ...
    test(testNamePrefix + "should add and replace multiple fields in the given order") {
    ...
    test(testNamePrefix + "should return any null fields in struct during add operation") {
      differentDataTypes.foreach { dataType =>
        ...
        val unsafeRowStruct = Literal.create(create_unsafe_row(schema.fields.map(_.dataType),
          Array(1, null)), schema)
    ...
    test(testNamePrefix + "should return any null fields in struct during replace operation") {
      differentDataTypes.foreach { dataType =>
        ...
        val expectedSchema = StructType(schema.updated(0, StructField("a", IntegerType,
          nullable = false)))
    ...
    test(testNamePrefix + "should handle attribute references when adding a field") {
      nonNullInputs.foreach { literalValue =>
        val value = literalValue.value
        val attributeReference = AttributeReference("a", literalValue.dataType,
          literalValue.nullable)().at(0)
    ...
    test(testNamePrefix + "should handle attribute references when replacing a field") {
      nonNullInputs.foreach { literalValue =>
        val value = literalValue.value
        val attributeReference = AttributeReference("a", literalValue.dataType,
          literalValue.nullable)().at(0)
    ...
    test(testNamePrefix + "should handle attribute references when adding multiple fields") {
      ...
      val expectedSchema = schema.add(StructField("d", IntegerType, nullable = false)).add(
    ...
    test(testNamePrefix + "should handle attribute references when replacing multiple fields") {
    ...
    test(testNamePrefix + "should replace all fields with given name") {
      ...
      val expectedSchema = StructType(Seq(StructField("a", IntegerType, nullable = false),
