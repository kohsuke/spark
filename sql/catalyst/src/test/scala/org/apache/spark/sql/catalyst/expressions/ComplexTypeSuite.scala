...
class ComplexTypeSuite extends SparkFunSuite with ExpressionEvalHelper {
...

  {
    ...
    test(testNamePrefix + "should fail if fieldName is null") {
      assert({
        val result = AddFields(nonNullStruct :: null :: Literal.create(2, IntegerType) :: Nil)
          .checkInputDataTypes()
        val expected = TypeCheckResult.TypeCheckFailure(
      ...
      nonNullInputs.foreach {
        case inputField if inputField.dataType.typeName != StringType.typeName =>
          assert({
            ...
            val expected = TypeCheckResult.TypeCheckFailure(
              s"Only non-null foldable ${StringType.catalogString} expressions are allowed to" +
                s" appear at even position.")
            result == expected
          })
        case inputField =>
          assert({
