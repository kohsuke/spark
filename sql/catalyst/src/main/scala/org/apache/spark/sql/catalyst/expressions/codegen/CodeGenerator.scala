...
class CodegenContext {
  ...
  var structuralSubExprEliminationExprssubExprEliminationExprs =
    Map.empty[Expression, SubExprEliminationState]
  ...
  private def structuralSubexpressionElimination(expressions: Seq[Expression]): Unit = {
    ...
    structuralExprs.flatMap { case (key, exprMap) =>
      ...
      val value = addMutableState(s"${javaType(expr.dataType)}[]", "subExprValue",
