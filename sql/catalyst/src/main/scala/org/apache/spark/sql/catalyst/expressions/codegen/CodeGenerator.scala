...
import org.apache.spark.sql.catalyst.util.{ArrayData, GenericArrayData, MapData, OrderingUtil}
...
class CodegenContext extends Logging {
  ...
  def genComp(dataType: DataType, c1: String, c2: String): String = dataType match {
    ...
    case DoubleType =>
      val clsName = OrderingUtil.getClass.getName.stripSuffix("$")
      s"$clsName.compareDoublesSQL($c1, $c2)"
    case FloatType =>
      val clsName = OrderingUtil.getClass.getName.stripSuffix("$")
      s"$clsName.compareFloatsSQL($c1, $c2)"
