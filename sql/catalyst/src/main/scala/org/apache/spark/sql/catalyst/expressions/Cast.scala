...
abstract class CastBase extends UnaryExpression with TimeZoneAwareExpression with NullIntolerant {
  ...
  private[this] def castToInterval(from: DataType): Any => Any = from match {
    case StringType if SQLConf.get.getConf(SQLConf.LEGACY_ALLOW_LEADING_INTERVAL_STRING_IN_CAST) =>
  ...
  private[this] def castToIntervalCode(from: DataType): CastFunction = from match {
    case StringType =>
      ...
      val func = if (SQLConf.get.getConf(SQLConf.LEGACY_ALLOW_LEADING_INTERVAL_STRING_IN_CAST)) {
