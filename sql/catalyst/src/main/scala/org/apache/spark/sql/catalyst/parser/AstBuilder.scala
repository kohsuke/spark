...
class AstBuilder(conf: SQLConf) extends SqlBaseBaseVisitor[AnyRef] with Logging {
  ...
  private def partitionExpressions(
    ...
    if (partTransforms.nonEmpty) {
      ...
      partCols.map { column =>
        IdentityTransform(FieldReference(column.name))
      }
    }
  }
    ...
    Option(ctx.query).map(plan) match {
      ...
      case Some(query) =>
        CreateTableAsSelectStatement(
          ...
          serdeInfo, external = external, ifNotExists = ifNotExists)
      ...
      case _ =>
        CreateTableStatement(table, schema.getOrElse(new StructType), partitioning, bucketSpec,
          properties, provider, options, location, comment, serdeInfo, external = external,
    ...
    Option(ctx.query).map(plan) match {
      ...
      case Some(query) =>
        ReplaceTableAsSelectStatement(table, query, partitioning, bucketSpec, properties,
          provider, options, location, comment, serdeInfo, orCreate = orCreate)
