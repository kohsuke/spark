  ...
  val STORE_ASSIGNMENT_POLICY =
    buildConf("spark.sql.storeAssignmentPolicy")
      .doc("When inserting a value into a column with different data type, Spark will perform " +
        ...
        "strict. With legacy policy, Spark allows casting any value to any data type and null " +
        "result is returned when the conversion is invalid. The legacy policy is the only " +
        "behavior in Spark 2.x and it is compatible with Hive. With strict policy, Spark doesn't " +
        "allow any possible precision loss or data truncation in type coercion, e.g. " +
        "`int` and `long`, `float` -> `double` are not allowed."
      )
      .stringConf
      .transform(_.toUpperCase(Locale.ROOT))
