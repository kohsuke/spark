...
object SQLConf {
  ...
  val PYSPARK_JVM_STACK_TRACE_ENABLED =
    buildConf("spark.sql.pyspark.jvmStackTrace.enabled")
      .doc("When true, it shows the JVM stack trace in the user-facing PySpark exception " +
        "together with Python stack trace. By default, it is disabled and hides JVM stacktrace " +
...
class SQLConf extends Serializable with Logging {
  ...
  def pysparkJVMStackTraceEnabled: Boolean = getConf(PYSPARK_JVM_STACK_TRACE_ENABLED)
