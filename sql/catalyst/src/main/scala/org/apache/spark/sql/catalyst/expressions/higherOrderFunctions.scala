...
@ExpressionDescription(
  ...
  examples = """
    Examples:
      > SELECT _FUNC_(array(5, 6, 1), (left, right) -> If(And(IsNull(left), IsNull(right)), 0, If(IsNull(left), 1, If(IsNull(right), -1, If(LessThan(left, right), -1, If(GreaterThan(left, right), 1, 0)))));
       [1,5,6]
      > SELECT _FUNC_(array('bc', 'ab', 'dc'), (left, right) -> If(And(IsNull(left), IsNull(right)), 0, If(IsNull(left), 1, If(IsNull(right), -1, If(LessThan(left, right), -1, If(GreaterThan(left, right), 1, 0)))));
       ["dc","bc","ab"]
