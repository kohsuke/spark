...
import org.apache.spark.sql.catalyst.util.OrderingUtil
...
private[sql] object FloatExactNumeric extends FloatIsFractional {
  ...
  override def compare(x: Float, y: Float): Int = OrderingUtil.compareFloatsSQL(x, y)
}
...
private[sql] object DoubleExactNumeric extends DoubleIsFractional {
  ...
  override def compare(x: Double, y: Double): Int = OrderingUtil.compareDoublesSQL(x, y)
}
