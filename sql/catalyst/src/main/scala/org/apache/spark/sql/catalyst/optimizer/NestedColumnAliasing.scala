...
object NestedColumnAliasing {
  ...
  def getAliasSubMap(exprList: Seq[Expression], exclusiveAttrs: Seq[Attribute] = Seq.empty)
    ...
    val aliasSub = nestedFieldReferences.asInstanceOf[Seq[ExtractValue]]
      ...
      // The above groupBy is to solve the following:
      // multiple `t.references.head` in `nestedFieldReferences` can have the same `exprId`
      // due to different `qualifiers`, making group by `t.references.head` unsafe.
