...
object NestedColumnAliasing {
  ...
  def getAliasSubMap(exprList: Seq[Expression], exclusiveAttrs: Seq[Attribute] = Seq.empty)
    ...
    val aliasSub = nestedFieldReferences.asInstanceOf[Seq[ExtractValue]]
      ...
      // By doing `groupBy` based on `exprId` and `dataType`,
      // we can ensure the result map correctly generates aliases map,
      // not only returning partial result like `exprIdA -> List(a, b)`.
