...
class ScriptTransformationSuite extends SparkPlanTest with SQLTestUtils with TestHiveSingleton
  ...
  test("SPARK-30973: TRANSFORM should wait for the termination of script (no serde)") {
    assume(TestUtils.testCommandAvailable("/bin/bash"))
  ...
  test("SPARK-30973: TRANSFORM should wait for the termination of script (with serde)") {
    assume(TestUtils.testCommandAvailable("/bin/bash"))
