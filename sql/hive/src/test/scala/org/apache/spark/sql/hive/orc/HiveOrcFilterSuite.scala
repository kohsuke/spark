...
class HiveOrcFilterSuite extends OrcTest with TestHiveSingleton {
  ...
  test("filter pushdown - decimal") {
    withOrcDataFrame((1 to 4).map(i => Tuple1.apply(BigDecimal.valueOf(i)))) { implicit df =>
      ...
      checkFilterPredicate(Literal(BigDecimal.valueOf(1)) === $"_1", PredicateLeaf.Operator.EQUALS)
      checkFilterPredicate(Literal(BigDecimal.valueOf(1)) <=> $"_1",
        PredicateLeaf.Operator.NULL_SAFE_EQUALS)
      checkFilterPredicate(Literal(BigDecimal.valueOf(2)) > $"_1", PredicateLeaf.Operator.LESS_THAN)
      checkFilterPredicate(Literal(BigDecimal.valueOf(3)) < $"_1",
        PredicateLeaf.Operator.LESS_THAN_EQUALS)
      checkFilterPredicate(Literal(BigDecimal.valueOf(1)) >= $"_1",
        PredicateLeaf.Operator.LESS_THAN_EQUALS)
      checkFilterPredicate(Literal(BigDecimal.valueOf(4)) <= $"_1",
